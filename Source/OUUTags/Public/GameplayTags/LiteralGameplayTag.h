...
namespace OUUTags::Private
{
	...
	/**
	 * Workaround that allows us to is std::is_same_v in combination with TOr and TAnd, because those expect ::Value to
	 * be upper case.
	 */
...
struct TIsChildTagOf_Single
{
	...
	static constexpr bool Value =
		TOr<OUUTags::Private::TIsSameWrapper<typename Child::SelfTagType, TestParent>,
			TAnd<
				TNot<OUUTags::Private::TIsSameWrapper<typename Child::SelfTagType, ParentOrExtendedTag>>,
...
{
#if WITH_EDITOR
	OUUTAGS_API void RegisterLiteralTagFlagsForValidation(
		FGameplayTag Tag,
		ELiteralGameplayTagFlags Flags,
		FName PluginName,
		FName ModuleName);
	OUUTAGS_API const TMap<FGameplayTag, ELiteralGameplayTagFlags>& GetTagFlagsForValidation();
#endif
} // namespace OUUTags::Private
...
namespace OUUTags::Private
{
	...
	public:
		...
		{
#if WITH_EDITOR
			OUUTags::Private::RegisterLiteralTagFlagsForValidation(
				Get(),
				Flags,
