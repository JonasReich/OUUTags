...
#include "GameplayTags/GameplayTagValidator.h"
...
void UGameplayTagValidationSettings::RefreshNativeTagOverrides()
{
	...
	for (auto& Entry : OUUTags::Private::GetTagFlagsForValidation())
	{
		if (static_cast<bool>(Entry.Value & ELiteralGameplayTagFlags::Inherited))
		{
...
void UGameplayTagValidatorSubsystem::ValidateGameplayTagTree()
{
	...
	if (Validators.Num() == 0)
	{
		UE_LOG(LogOUUTags, Error, TEXT("No non-abstract gameplay tag validator classes found."));
		return;
	}
	...
	{
		MessageLog.Message(EMessageSeverity::Error, Error);
	}
	for (const auto& Warning : Warnings)
	{
		MessageLog.Message(EMessageSeverity::Warning, Warning);
	}

	if (Errors.Num() > 0)
	{
		MessageLog.Notify(FText::Format(INVTEXT("{0} GameplayTag validation errors"), Errors.Num()));
	}
	else if (Warnings.Num() > 0)
	{
		MessageLog.Notify(FText::Format(INVTEXT("{0} GameplayTag validation warnings"), Warnings.Num()));
	}
	else
	{
		MessageLog.Notify(INVTEXT("No GameplayTag validation issues found."));
	}
}
